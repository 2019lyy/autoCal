...
def logStart():
	...
	#切换到history目录
	if (os.getcwd().split('\\')[-1] == 'autoCal'):
		os.chdir('./instance/history')
		os.makedirs(nowtime)
		os.chdir('./' + nowtime)
	else:
		os.chdir('../../history')
...
def init_env():
	...
	#变量加载完毕，开始加载全局工况
	if (settings.debug_mode):
		logger.append('全局工况清空')
	global_entry.clear()
	if (settings.debug_mode):
		logger.append('全局工况加载中')
	...
	if (settings.debug_mode):
		logger.append('全局工况加载完成')
	#加载完毕后输出测试
	# for key in global_entry:
	# 	for temp in global_entry[key]:
	# 		logger.append('全局工况名称:' + temp.get('c_name') + ' ，执行频率:' + str(key))

	#全局工况开始运行
	runGlobalEntry()
	if (settings.debug_mode):
		logger.append('全局开始运行')
...
def getRecord(record_name):
	if (os.getcwd().split('\\')[-1] == 'autoCal'):
		os.chdir('./instance/history/' + record_name)
	else:
		os.chdir('../../history/' + record_name)
